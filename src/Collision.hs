...
handleCollision gstate@(GameState i e p c u s l b r sc Playing ) =
    let playerCollision  = handlePlayerCollision p gstate
        newBullets       = map (`handleBulletCollisions` gstate) b
        cometCollision   = map (`handleCometCollision` gstate) c
        scatterCollision = map (`handleScatterCollision` gstate) s
        ufoCollision     = map (`handleUfoCollision` gstate) u
...
handleUfoCollision u gstate@GameState{..}    | (invincible u == False) || checkCollision u bullets  || collide u player = u {uLives = uLives u - 1}
                                             | otherwise = u
